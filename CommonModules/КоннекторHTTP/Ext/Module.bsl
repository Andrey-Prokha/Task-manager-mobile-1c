...
Функция Скопировать(Источник)
	...
	#Если МобильноеПриложениеСервер Тогда
		...
		ПараметрыПреобразованияJSON.ПрочитатьВСоответствие = ТипЗнч(Источник) = Тип("Соответствие");
		Возврат JsonВОбъект(ОбъектВJson(Источник),, ПараметрыПреобразованияJSON);
	#Иначе
